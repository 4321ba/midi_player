...
def parse_arguments():
    ...
    parser.add_argument("input_file", nargs="?", default = "", help="midi file to open, leave blank for random in current dir")
    parser.add_argument("-l", "--loop", type=int, default = 1, metavar="count", help="play more midi files")
    parser.add_argument("-s", "--speed", type=float, default = 1, metavar="multiplier", help="multiplies the playback speed")
...
def select_input_file(input_file):
    if input_file == "":
        midi_files = [i for i in os.listdir() if i[-4:] == ".mid"]
...
def calculate_hz(midi_note):
    if midi_note < 45:
        return int(440*2**((midi_note-69)/12))
    else:
        return int(440*2**((midi_note-69)/12)*(((midi_note-45)*0.005)**2+1))

def restrict_number_of_notes(data, pins): #data: [[s az előzőtől, on-e, hgmagasság],[]]
    ...
    for i in data:
        if i[1]:
            if len(pins) - 1 == len(sounding_notes):
                ...
                new_data.append([i[0] + delay_until_good_off, shifted_note[0], calculate_hz(i[2])])
                delay_until_good_off = 0
                bad_offs_count += 1
            else:
                for j in pins:
                    if not any(j == k[0] for k in sounding_notes):
                        ...
                        new_data.append([i[0] + delay_until_good_off, j, calculate_hz(i[2])])
                        delay_until_good_off = 0
                        break
        else:
            if i[2] in bad_offs:
...
def main():
    ...
    for i in range(args["loop"]):
        ...
        input_file = select_input_file(args["input_file"])
        data = import_midi_file(input_file, args["is_silent"])
        ...
        data, stats["bad_offs_count"] = restrict_number_of_notes(data, pins)
